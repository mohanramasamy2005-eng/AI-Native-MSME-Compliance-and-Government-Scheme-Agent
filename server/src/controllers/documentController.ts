...
const field = (label: string, value: string, confidence: number) => ({
  label,
  value,
  confidence,
});
...
function analyseText(text: string, fileName: string, profile: any) {
  const source = text || fileName;
  const lower = source.toLowerCase();
  const dates = [...source.matchAll(datePattern)].map((match) =>
    normaliseDate(match[0]),
  );
  const expiryMatch = source.match(
    /(?:expiry|valid until|valid upto|expires?)[^\d]*(\d{2}[-/]\d{2}[-/]20\d{2}|20\d{2}[-/]\d{2}[-/]\d{2})/i,
  );
  ...
  for (const [label, pattern] of patterns) {
    ...
    if (match) extractedFields.push(field(label, match[0], 0.9));
  }
  if (expiryDate) extractedFields.push(field("Expiry Date", expiryDate, 0.86));
  if (dates.length > 0)
  ...
  if (!text || text.length < 30)
    missingInformation.push(
      "Readable text could not be reliably extracted; manual review is required.",
    );
  if (!expiryDate)
  ...
  if (missingInformation.length)
    recommendations.push(
      "Resolve missing or unreadable fields and upload a clearer copy if needed.",
    );
  if (expiryDate)
    recommendations.push(
      `Plan renewal before ${expiryDate}; this date is extracted, not authenticated.`,
    );
...
export const uploadDocumentMetadata = async (req: Request, res: Response) => {
  try {
    ...
    const rag = await searchGovernmentKnowledgeBase(
      `${analysis.documentType} expiry renewal document requirements`,
    );
    ...
    const newDoc = await DocumentModel.create({
      ...
      fileSize: file
        ...
        : "1.5 MB",
      status:
        analysis.potentialIssues.length || analysis.missingInformation.length
          ? "Action Needed"
          : "Analyzed",
      confidenceScore: Math.max(
        ...
        Math.min(
          98,
          Math.round(
            (analysis.extractedFields.reduce(
      ...
      review: {
        ...
        ragGuidance: rag.map(
          (citation) =>
            `${citation.documentTitle}: ${citation.matchedText.slice(0, 240)}`,
        ),
      },
    });
      ...
      if (!Number.isNaN(dueDate.getTime()) && daysRemaining >= 0) {
        ...
        await ActionItem.findOneAndUpdate(
          ...
          },
          {
            ...
            priority:
              deadline?.priority || (daysRemaining <= 30 ? "high" : "medium"),
            status: "Pending",
            dueDate: analysis.expiryDate,
